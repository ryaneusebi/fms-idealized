"""
Spectral analysis module for FMS model output.

This module provides functions to compute various spectral diagnostics including:
- Spherical harmonic decomposition of fields
- Barotropic and baroclinic components of spectra
- Spectra on pressure levels and latitude lines
- Cospectra for eddy fluxes
"""

import xarray as xr
import pyshtools as pysh
import numpy as np
from cospectra import compute_eddy_cospectra
import matplotlib.pyplot as plt
from utils import get_zavg, get_pderiv, get_phideriv
import dask

def spectral_analysis(ucomp, vcomp, theta, ps, phalf, r, omega, chunks, meanstats=None, apply_smoothing=False, smooth_width=3):
    """
    ...
    # rechunk for spectral calculations
    up = up.chunk(chunks)
    vp = vp.chunk(chunks)
    thetap = thetap.chunk(chunks)
    qp = qp.chunk(chunks)
    ...
    eke_baroc_spectrum_full = spherical_spectrum_full(up_bc, vp_bc).rename("eke_bc_spectrum_sigma")

    u_spectrum_full = spherical_spectrum_full(up, up).rename("u_spectrum_sigma")
    v_spectrum_full = spherical_spectrum_full(vp, vp).rename("v_spectrum_sigma")
    theta_spectrum_full = spherical_spectrum_full(thetap, thetap).rename("theta_spectrum_sigma")
    ...
    u_baroc_spectrum_full = spherical_spectrum_full(up_bc, up_bc).rename("u_bc_spectrum_sigma")
    v_baroc_spectrum_full = spherical_spectrum_full(vp_bc, vp_bc).rename("v_bc_spectrum_sigma")
    theta_baroc_spectrum_full = spherical_spectrum_full(thetap_bc, thetap_bc).rename("theta_bc_spectrum_sigma")
    ...
    eke_baroc_xspectrum_full = zonal_energy_spectrum(up_bc, vp_bc).rename("eke_bc_xspectrum_sigma")
    ...
    u_baroc_xspectrum_full = zonal_power_spectrum(up_bc).rename("u_bc_xspectrum_sigma")
    ...
    v_baroc_xspectrum_full = zonal_power_spectrum(vp_bc).rename("v_bc_xspectrum_sigma")
    ...
    theta_xspectrum_full = zonal_power_spectrum(thetap).rename("theta_xspectrum_sigma")
    theta_barot_xspectrum = zonal_power_spectrum(thetap_zavg).rename("theta_bt_xspectrum")
    theta_baroc_xspectrum_full = zonal_power_spectrum(thetap_bc).rename("theta_bc_xspectrum_sigma")
    ...
    (eke_spectrum, eke_barot_spectrum, eke_baroc_spectrum, 
        u_spectrum, u_barot_spectrum, u_baroc_spectrum,
        v_spectrum, v_barot_spectrum, v_baroc_spectrum,
        theta_spectrum, theta_barot_spectrum, theta_baroc_spectrum) = dask.compute(
            eke_spectrum, eke_barot_spectrum, eke_baroc_spectrum,
            u_spectrum, u_barot_spectrum, u_baroc_spectrum,
            v_spectrum, v_barot_spectrum, v_baroc_spectrum,
    ...
    (eke_spectrum_full, eke_baroc_spectrum_full,
                             u_spectrum_full, u_baroc_spectrum_full,
                             v_spectrum_full, v_baroc_spectrum_full,
                             theta_spectrum_full, theta_baroc_spectrum_full) = dask.compute(
            eke_spectrum_full, eke_baroc_spectrum_full,
            u_spectrum_full, u_baroc_spectrum_full,
            v_spectrum_full, v_baroc_spectrum_full,
    ...
    # rechunk for cospectral analysis
    
    cospectra_thetav = cospectra_eddyflux(thetap, vp, r, apply_smoothing=apply_smoothing, smooth_width=smooth_width)
    cospectra_thetav = cospectra_thetav.rename("cospectra_thetav")
    del(thetap)

    cospectra_uv = cospectra_eddyflux(up, vp, r, apply_smoothing=apply_smoothing, smooth_width=smooth_width)
    cospectra_uv = cospectra_uv.rename("cospectra_uv")
    del(up)

    cospectra_qv = cospectra_eddyflux(qp, vp, r, apply_smoothing=apply_smoothing, smooth_width=smooth_width)
    cospectra_qv = cospectra_qv.rename("cospectra_qv")
    del(qp)

    cospectra_thetav, cospectra_uv, cospectra_qv = dask.compute(
        cospectra_thetav, cospectra_uv, cospectra_qv
    )
...
# inputs are two vector fields on the same grid
def spherical_spectrum_full(u, v):
    """
    Calculate spherical harmonic spectrum at each pressure level (sigma).
    Efficient xarray version using apply_ufunc, vectorized over sigma.
    """
    n_l = len(u.lat)//2

    def _ke_spec(u2d, v2d):
        u_spectrum = spherical_spectrum(u2d)
        v_spectrum = spherical_spectrum(v2d)
        return 0.5 * (u_spectrum + v_spectrum)
    ...
    result = xr.apply_ufunc(
        ...
        dask="parallelized",
        output_dtypes=[float],
        dask_gufunc_kwargs={"output_sizes": {"l": n_l}}
    )
    result = result.assign_coords(l=("l", np.arange(n_l)))
    if 'time' in result.dims:
        result = result.mean("time")
    if "sigma" in u.dims:
        result = result.transpose("sigma", "l")
...
def spherical_spectrum(x):
    ...
    """
    grid_x = pysh.SHGrid.from_array(x)
...
def spherical_energy_spectrum(u, v):
    ...
    Efficient xarray version using apply_ufunc, vectorized over time.
    """
    def _ke_spec(u2d, v2d):
        u_spectrum = spherical_spectrum(u2d)
        v_spectrum = spherical_spectrum(v2d)
...
def spherical_power_spectrum(x):
    ...
    if "time" in result.dims:
        result = result.mean("time")
    result = result.assign_coords(l=("l", np.arange(1,n_l+1)))
    return result
...
def cospectra_eddyflux(varp, vp, r, apply_smoothing=False, smooth_width=3):
    """
    Calculate cospectra of meridional eddy fluxes using compute_eddy_cospectra from cospectra_new.py.
    Passes xarray DataArrays directly and lets compute_eddy_cospectra handle parallelization over lat and sigma.
    """
    dt = 6 * 60 * 60  # 6 hours in seconds
    # compute_eddy_cospectra should be updated to handle (sigma, lat, time, lon) or (lat, time, lon)
    return compute_eddy_cospectra(varp, vp, r, dt, apply_smoothing=apply_smoothing, smooth_width=smooth_width)

def zonal_power_spectrum(x):
    """
    Calculate zonal wavenumber power spectrum using FFT along longitude.
    Works with Dask and NumPy arrays, vectorized over all other axes.
    Averages over time before returning.
    """
    nlon = x.sizes['lon']
    nk = nlon // 2

    def _zonal_fft(x1d):
        nlon_local = x1d.shape[-1]
        xf = np.fft.fft(x1d, axis=-1) / nlon_local
        power = np.abs(xf)**2
        nk_local = nlon_local // 2
        return power[..., :nk_local]*2
    ...
    result = xr.apply_ufunc(
        ...
        dask="parallelized",
        output_dtypes=[float],
        dask_gufunc_kwargs={"output_sizes": {"k": nk}},
    )

    if "time" in result.dims:
        result = result.mean("time")

    result = result.assign_coords(k=("k", np.arange(nk)))
    return result
